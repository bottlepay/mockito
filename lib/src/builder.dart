...
class _MockTarget {
  ...
  _MockTarget(
    ...
    required this.isInjectableMock,
  });
  ...
  static Iterable<_MockTarget> _mockTargetsFromGenerateMocks(
    ...
    if (customMocksField != null && !customMocksField.isNull) {
      for (var mockSpec in customMocksField.toListValue()!) {
        ...
        final isInjectableMock =
            mockSpec.getField('isInjectableMock')?.toBoolValue() ?? false;
        mockTargets.add(_MockTarget(type, mockName,
            returnNullOnMissingStub: returnNullOnMissingStub,
            isInjectableMock: isInjectableMock,
            fallbackGenerators:
    ...
    return Class((cBuilder) {
      cBuilder
        ...
        ..annotations = ListBuilder<Expression>([
          if (mockTarget.isInjectableMock)
            referImported('test', 'package:injectable/injectable.dart'),
          if (mockTarget.isInjectableMock)
            referImported('Singleton', 'package:injectable/injectable.dart')
                .call(
              [],
              {
                'as': referImported(
                  classToMock.name,
                  typeToMock.element.location?.components[0],
                )
              },
            ),
        ])
